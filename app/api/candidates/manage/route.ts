...
export async function GET(req:NextRequest){try{const auth=await getServerAuth(req,WRITE_ROLES);if(!auth)return NextResponse.json({error:'غير مصرح'},{status:401});const h=supabaseHeaders(auth);const rejected=new URL(req.url).searchParams.get('status')==='rejected';const statusFilter=rejected?'&status=eq.%D9%85%D8%B1%D9%81%D9%88%D8%B6':'&status=neq.%D9%85%D8%B1%D9%81%D9%88%D8%B6';const [c,r]=await Promise.all([fetch(`${SUPABASE_URL}/rest/v1/candidates?select=*&order=created_at.desc${statusFilter}`,{headers:h,cache:'no-store'}),fetch(`${SUPABASE_URL}/rest/v1/requests?select=id,exact_type,request_type`,{headers:h,cache:'no-store'})]);if(!c.ok||!r.ok)throw new Error('تعذر تحميل المرشحين');return NextResponse.json({candidates:await c.json(),requests:await r.json()},{headers:{'Cache-Control':'no-store'}})}catch(e){return NextResponse.json({error:e instanceof Error?e.message:'تعذر تحميل المرشحين'},{status:500})}}
