...
async function adminRequest(path: string, init: RequestInit = {}) {
  ...
  if (!serviceKey) throw new Error('لم يتم إعداد مفتاح إدارة المستخدمين على الخادم.')
...
export async function POST(req: NextRequest) {
  try {
    ...
    const profileResponse = await adminRequest(`/rest/v1/app_users?select=role,is_active&user_id=eq.${caller.id}`)
    ...
    if (caller.email.toLowerCase() === BOOTSTRAP_EMAIL && !profiles.length) {
      await adminRequest('/rest/v1/app_users', {
        method: 'POST',
        headers: { Prefer: 'return=minimal' },
        body: JSON.stringify({ user_id: caller.id, display_name: 'مسؤول الموارد البشرية', role: 'admin', is_active: true }),
      })
    }
    ...
    if (!createResponse.ok || !created?.id) {
      return NextResponse.json({ error: created?.msg || created?.message || 'تعذر إنشاء المستخدم.' }, { status: 400 })
    }

    const profileInsert = await adminRequest('/rest/v1/app_users', {
      method: 'POST',
      headers: { Prefer: 'return=minimal' },
      body: JSON.stringify({ user_id: created.id, display_name: displayName, role, is_active: true }),
    })
