...
export default function LoginPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      // Navigate only after the HttpOnly cookie has been successfully created.
    ...
    <main dir="rtl" className="min-h-screen relative overflow-hidden bg-[#071a2d] flex items-center justify-center px-4 py-8">
      ...
      <div className="relative z-10 w-full max-w-5xl grid lg:grid-cols-[1fr_1.05fr] gap-8 items-center">
        <section className="hidden lg:block text-white px-8">
          <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-4 py-2 text-sm text-white/75 backdrop-blur"><Sparkles size={16} className="text-[#d4a72c]" />منصة إدارية متكاملة</div>
          ...
          <div className="flex items-center gap-5 mt-10 text-sm text-white/55"><span className="flex items-center gap-2"><ShieldCheck size={18} className="text-[#d4a72c]" /> وصول محمي</span><span className="flex items-center gap-2"><LockKeyhole size={18} className="text-[#d4a72c]" /> جلسات آمنة</span></div>
        </section>
        ...
        <section className="w-full max-w-md mx-auto bg-white/95 backdrop-blur-xl rounded-[2rem] p-7 md:p-9 shadow-[0_30px_80px_rgba(0,0,0,.35)] border border-white/30">
          <div className="text-center mb-7"><div className="mx-auto w-20 h-20 rounded-[1.6rem] bg-[#09233f] text-[#d4a72c] flex items-center justify-center shadow-lg shadow-[#09233f]/20"><ShieldCheck size={43} strokeWidth={1.8} /></div><h2 className="text-3xl font-black text-[#09233f] mt-5">مرحباً بك</h2><p className="text-slate-500 mt-2">سجّل الدخول للوصول إلى النظام</p></div>
          <div className="flex items-start gap-3 mb-6 p-4 rounded-2xl bg-[#f7f9fc] border border-slate-200"><div className="w-10 h-10 rounded-xl bg-[#09233f] text-[#d4a72c] flex items-center justify-center shrink-0"><LockKeyhole size={19} /></div><div><div className="font-black text-[#09233f]">دخول آمن</div><div className="text-xs text-slate-500 mt-1 leading-5">هذه البوابة مخصصة للمستخدمين المعتمدين فقط.</div></div></div>
          <form onSubmit={submit} className="space-y-5">
            <label className="block text-sm font-bold text-[#09233f]">البريد الإلكتروني<input type="email" value={email} onChange={e => setEmail(e.target.value)} autoComplete="username" required className="mt-2 w-full border border-slate-300 focus:border-[#b88618] focus:ring-4 focus:ring-[#d4a72c]/10 outline-none rounded-xl px-4 py-3.5 bg-white transition" placeholder="name@company.com" /></label>
            <label className="block text-sm font-bold text-[#09233f]">كلمة المرور<div className="relative mt-2"><input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} autoComplete="current-password" required className="w-full border border-slate-300 focus:border-[#b88618] focus:ring-4 focus:ring-[#d4a72c]/10 outline-none rounded-xl px-4 py-3.5 pl-12 bg-white transition" placeholder="أدخل كلمة المرور" /><button type="button" onClick={() => setShowPassword(v => !v)} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-[#09233f] transition" aria-label={showPassword ? 'إخفاء كلمة المرور' : 'إظهار كلمة المرور'}>{showPassword ? <EyeOff size={19} /> : <Eye size={19} />}</button></div></label>
            {error && <div className="rounded-xl bg-red-50 border border-red-200 text-red-700 p-3.5 text-sm font-bold leading-6">{error}</div>}
            <button type="submit" disabled={loading} className="w-full bg-[#09233f] hover:bg-[#12385d] disabled:opacity-60 text-white rounded-xl px-5 py-3.5 font-black flex items-center justify-center gap-2 transition shadow-lg shadow-[#09233f]/15"><LogIn size={19} />{loading ? 'جاري التحقق...' : 'تسجيل الدخول'}{!loading && <ArrowLeft size={17} />}</button>
          </form>
