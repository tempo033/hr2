...
export default function EmployeeImportPage() {
  ...
  const downloadTemplate=()=>{
    const ws=XLSX.utils.aoa_to_sheet([columns,['EMP-1001','محمد أحمد','مصري','1234567890','2026-01-01','على الكفالة',5000,500,1000,250,6750]])
    ws['!cols']=columns.map(()=>({wch:24}))
    // SheetJS community writer cannot reliably persist Excel list validation.
    // The template therefore documents the allowed values in a second sheet.
    const statusWs=XLSX.utils.aoa_to_sheet([['القيم المسموح بها في خانة حالة العامل'],...workerStatuses.map(x=>[x])])
    statusWs['!cols']=[{wch:34}]
    const wb=XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb,ws,'بيانات الموظفين'); XLSX.utils.book_append_sheet(wb,statusWs,'اختيارات حالة العامل'); XLSX.writeFile(wb,'نموذج_ملفات_الموظفين.xlsx')
  }
  ...
  const importFile=async(e:React.ChangeEvent<HTMLInputElement>)=>{
    ...
    try {
      ...
      const data=rows.slice(1).filter(r=>String(r[idx[0]]||'').trim() || String(r[idx[1]]||'').trim()).map((r,rowIndex)=>({
